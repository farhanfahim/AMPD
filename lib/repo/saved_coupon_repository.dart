...
class SavedCouponRepository {
  ...
  factory SavedCouponRepository({@required AppPreferences appPreferences}) =>
      SavedCouponRepository._internal(appPreferences);

  SavedCouponRepository._internal(this._appPreferences);
  ...
  void deleteOffer(Map<String, dynamic> map){
    ...
    _appPreferences.getAccessToken().then((value) {
      ...
      NetworkNAO.deleteOffer(value, map).then((response) async {
        final data = (response as Response<dynamic>).data;
        if (!data['status']) {
          repositoryResponse.success = false;
          repositoryResponse.msg = data['message'];
